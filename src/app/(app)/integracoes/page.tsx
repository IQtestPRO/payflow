    ...
    <div className="grid gap-6">
      ...
      <section>
        <div className="mb-3 flex flex-col gap-1 md:flex-row md:items-end md:justify-between">
          <div>
              <h2 className="text-xl font-extrabold">Conectores principais</h2>
      ...
      <section className="data-panel p-5">
        <div className="flex flex-col gap-1 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-xl font-extrabold">Canais, pixel, API e automações</h2>
          </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {supportingIntegrationBrands.map((item) => (
            <article key={item.label} className="rounded-lg border border-border/80 bg-white/[0.96] p-4 shadow-inner-line transition duration-200 hover:-translate-y-0.5 hover:border-primary/25 hover:shadow-soft">
              <div className="flex items-center gap-3">
...
function SummaryMetric({ label, value }: { label: string; value: number }) {
  return (
    <div className="border-l border-t border-white/10 p-4 first:border-l-0">
      <p className="text-xs font-semibold uppercase tracking-normal text-white/60">{label}</p>
