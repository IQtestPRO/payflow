...
import type { OfferRecord, ProductRecord } from "@/lib/types";

export function OffersManager({ initialOffers, products }: { initialOffers: OfferRecord[]; products: ProductRecord[] }) {
  ...
  async function create(formData: FormData) {
    ...
    setOffers((current) => [json as OfferRecord, ...current]);
  }
  ...
  return (
    <div className="grid gap-4">
      <form action={create} className="surface grid gap-3 p-4 xl:grid-cols-4">
        ...
        <input className="field" name="salesPageUrl" placeholder="URL da página de vendas" />
        <input className="field" name="checkoutUrl" placeholder="URL do checkout" />
        <input className="field" name="trafficSourceDefault" placeholder="Origem padrão" />
        <input className="field" name="defaultUtmSource" placeholder="UTM source" />
        <input className="field" name="defaultUtmMedium" placeholder="UTM medium" />
        <input className="field" name="defaultUtmCampaign" placeholder="UTM campaign" />
        <button className="btn-primary xl:col-span-2" type="submit"><Plus className="h-4 w-4" aria-hidden="true" />Criar oferta</button>
      </form>
      {error ? <p className="rounded-md bg-red-50 p-3 text-sm font-medium text-red-700">{error}</p> : null}
      <DataTable headers={["Oferta", "Produto", "Preço", "Funil", "Métricas", "Status", "Ações"]}>
        {offers.map((offer) => (
          <tr key={offer.id} className="hover:bg-muted/50">
            ...
            <td className="px-4 py-3">{offer.productName ?? "Sem produto"}</td>
